...
The general algorithm for convergent series implemented below is as follows. Consider polynomials $P(x)$ and $Q(x)$ that have only unique roots (i.e. the multiplicity of each is 1). Denote the roots of the polynomial $Q(x+1)$ (obtained using the most convenient means) as $R_0 \dots R_m$. The following equality holds if $P(x)=1$:
