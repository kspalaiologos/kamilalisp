...
To implement a mean function using a fork, it is necessary to define a function that sums the list beforehand using the \verb|tally| function which returns the length of a list. A non-zero value is considered truthy, hence the following definition of the \textit{sum} function:
...
The key merits of point-free programming (a paradigm in which function definitions do not identify the arguments or "points" on which they operate) are:
...
\begin{Verbatim}
    ...
    (λ n . (if (= n 1) 1 (collatz-rec (collatz n))))
