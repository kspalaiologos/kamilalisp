    ...
    public Atom apply(Environment env, List<Atom> args) {
        if(args.size() == 2) {
            ...
            if (args.get(1).getType() == Type.LIST) {
                ...
                return new Atom(list.stream().allMatch(x -> Evaluation.evaluate(env, reductor, List.of(x)).coerceBool()));
            } else if (args.get(1).getType() == Type.STRING) {
                String str = args.get(1).getString();
                return new Atom(str.chars().allMatch(x -> Evaluation.evaluate(env, reductor, List.of(new Atom(String.valueOf((char) x)))).coerceBool()));
            } else {
                throw new UnsupportedOperationException("all not defined for: " + args.get(1).getType());
            }
        } else if(args.size() == 1) {
            List<Atom> list = args.get(0).getList();
            return new Atom(list.stream().allMatch(Atom::coerceBool));
        } else {
