...
public class ChordalGraphColouring extends PrimitiveFunction implements Lambda {
    @Override
    public Atom apply(Environment env, List<Atom> args) {
        assertArity(args, 1);
        GraphWrapper wp = args.get(0).getUserdata(GraphWrapper.class);
        var colouring = new ChordalGraphColoring<>(wp.getGraph());
        ...
        Map<Atom, Atom> data = Map.ofEntries(
                Map.entry(new Atom("chromatic-number"), new Atom(colouring.getPerfectEliminationOrder())),
    ...
    protected String name() {
        return "graph:chordal-colouring";
    }
}
