...
public class BitOr extends PrimitiveFunction implements Lambda {
    ...
    public static Atom bitop2(Atom a, Atom b) {
        ...
        if (a.getType() == Type.INTEGER && b.getType() == Type.INTEGER) {
            return new Atom(a.getInteger().or(b.getInteger()));
            ...
            return new Atom(Streams.zip(A.stream(), B.stream(), BitOr::bitop2).collect(Collectors.toList()));
    ...
    public Atom apply(Environment env, List<Atom> args) {
        if (args.size() == 2) {
            ...
            return args.stream().reduce(BitOr::bitop2).get();
        }
    }
