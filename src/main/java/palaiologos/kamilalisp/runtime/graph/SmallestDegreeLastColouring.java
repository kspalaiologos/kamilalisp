...
public class SmallestDegreeLastColouring extends PrimitiveFunction implements Lambda {
    @Override
    public Atom apply(Environment env, List<Atom> args) {
        assertArity(args, 1);
        GraphWrapper wp = args.get(0).getUserdata(GraphWrapper.class);
        var colouring = new SmallestDegreeLastColoring<>(wp.getGraph());
    ...
    protected String name() {
        return "graph:smallest-degree-last-colouring";
    }
}
