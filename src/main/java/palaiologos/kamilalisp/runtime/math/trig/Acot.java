...
public class Acot extends PrimitiveFunction implements Lambda {
    private static final String name = "acot";

    public static Atom trig1(Environment env, Atom a) {
        a.assertTypes(Type.REAL, Type.COMPLEX, Type.LIST);
        if(a.getType() == Type.COMPLEX) {
            return new Atom(BigComplexMath.acot(a.getComplex(), env.getMathContext()));
        } else if(a.getType() == Type.REAL) {
            return new Atom(BigDecimalMath.acot(a.getReal(), env.getMathContext()));
