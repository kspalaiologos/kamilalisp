...
public class BitAnd extends PrimitiveFunction implements Lambda {
    private static final String name = "bit-and";
    ...
    public static Atom bitop2(Atom a, Atom b) {
        ...
        if (a.getType() == Type.INTEGER && b.getType() == Type.INTEGER) {
            return new Atom(a.getInteger().and(b.getInteger()));
            ...
            return new Atom(Streams.zip(A.stream(), B.stream(), BitAnd::bitop2).collect(Collectors.toList()));
    ...
    public Atom apply(Environment env, List<Atom> args) {
        if (args.size() == 2) {
            ...
            return args.stream().reduce(BitAnd::bitop2).get();
        }
    }
