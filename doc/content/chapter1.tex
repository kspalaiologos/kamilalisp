...
This may alert the observant reader once again - \textit{What if I want to create a list out of a set of expressions}? This question is once again very relevant - because the quote stops a form from being evaluated, then surely the expressions inside of it will stay untouched too. The answer is simple - use the \verb|tie| function as follows:
...
Since \verb|square| is now bound in the global scope, it can be applied to an argument. The code below binds the name \verb|a| to the result of application of the number 5 to the function \verb|square|:
