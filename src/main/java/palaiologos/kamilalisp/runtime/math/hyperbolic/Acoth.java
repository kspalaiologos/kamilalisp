...
public class Acoth extends PrimitiveFunction implements Lambda {
    private static final String name = "acoth";

    private static BigComplex imagTrig(BigComplex z, MathContext mc) {
        // acoth(z) = 1/2 * ln((z + 1)/(z - 1))
        BigComplex x = z.add(BigComplex.ONE, mc);
        BigComplex y = z.subtract(BigComplex.ONE, mc);
        BigComplex result = BigComplexMath.log(x.divide(y, mc), mc);
        return result.divide(BigComplex.valueOf(2), mc);
    }
