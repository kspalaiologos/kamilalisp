...
public class UnGradeUp extends PrimitiveFunction implements Lambda {
    @Override
    public Atom apply(Environment env, List<Atom> args) {
        if (args.size() == 1) {
            return new Atom(Lists.reverse(IntStream.range(0, args.get(0).getList().size()).boxed().sorted(Comparator.comparing(o -> args.get(0).getList().get(o))).map(x -> new Atom(BigInteger.valueOf(args.get(0).getList().size() - x - 1))).toList()));
        } else if (args.size() == 2) {
